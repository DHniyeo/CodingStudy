...
int get_maxR(int idx) {
	...
	for (int i = 0; i < 4; i++) {
		int ny = blocks[idx].exit[blocks[idx].d].r + dy[i]; // 출구방향에서 4가지 방향으로 찾기
		int nx = blocks[idx].exit[blocks[idx].d].c + dx[i];
		if (ny >= R + 3 || nx < 0 || nx >= C) continue;
		if (map[ny][nx] == 0 || map[ny][nx] == idx + 1) continue;
		nowMaxR = nowMaxR < blocks[map[ny][nx] - 1].maxV ? blocks[map[ny][nx] - 1].maxV : nowMaxR;
	}
...
int main() {
	...
	for (int tc = 0; tc < K; tc++) { // 정령의 수
		...
		infinite_move(tc);
		// 만약 0~2 행에 값이 존재한다면 맵을 비우고 해당번호 다시 무한 이동 작업을함
		if (check()) {
